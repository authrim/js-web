...
describe("Silent Login Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(window, "crypto", {
      value: {
        ...
        randomUUID: vi.fn(() => "silent-state-123"),
      },
      writable: true,
      configurable: true,
    });
  ...
  // ===========================================================================
  // trySilentLogin() - Top-Level prompt=none Handoff Navigation
  // ===========================================================================

  describe("trySilentLogin() - top-level navigation contract", () => {
    it("should redirect with prompt=none and handoff=true and store same-origin return state", async () => {
      ...
      const redirectUrl = new URL((window.location as any).href);
      ...
      expect(state).toBe("silent-state-123");
      expect(JSON.parse(mockStorage[`authrim:silent_login:${state}`])).toEqual({
        t: "sl",
        lr: "r",
  ...
  describe("handleSilentCallback() - sessionStorage state retrieval", () => {
    ...
    it("should return not_silent_login error when state not in sessionStorage", async () => {
      ...
      (window.location as any).search =
        "?state=regular-oauth-state&code=abc123";
    ...
    it("should return invalid_state_data error when missing type marker", async () => {
      ...
      mockStorage[`authrim:silent_login:${state}`] =
  ...
  describe("handleSilentCallback() - OAuth error handling", () => {
    it("should handle consent_required error", async () => {
      ...
      const state = "test-state-123";
      (window.location as any).search =
        `?state=${state}&error=consent_required`;
    ...
    it("should handle error with error_description", async () => {
      ...
      const state = "test-state-123";
      (window.location as any).search =
        `?state=${state}&error=access_denied&error_description=User%20denied%20access`;
    ...
    it("should handle interaction_required error", async () => {
      ...
      const state = "test-state-123";
      (window.location as any).search =
        `?state=${state}&error=interaction_required`;
